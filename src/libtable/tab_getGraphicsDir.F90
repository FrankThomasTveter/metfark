subroutine tab_getgraphicsdir(pid, gra250, crc250, irc)
  use table
  implicit none
  ...
  integer :: irc
  character*25 :: myname = "getGraphicsdir"
  ...
  call table_getgraphicsdir(pss,gra250,crc250,irc)
  if (irc.ne.0) then
     call table_errorappend(crc250,myname)
     call table_errorappend(crc250," Error return from tab_getgraphicsdir.")
     call table_errorappendi(crc250,irc)
     call table_errorappend(crc250,"\n")
     return
  end if
  !write(*,*) myname,' Done.'
  return
end subroutine tab_getgraphicsdir
