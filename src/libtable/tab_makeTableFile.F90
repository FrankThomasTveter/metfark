subroutine tab_maketablefile(sid,cid,mid,oid,tab250,gra250,cat250,test,fill250,crc250, irc)
  use table
  use model
  use observations
  use colocation
  ...
  integer :: irc
  character*25 :: myname = "tab_maketableFile"
  ...
  !
  call  table_maketablefile(pss,css,mss,oss,tab250,gra250,cat250,test,fill250,crc250,irc)
  if (irc.ne.0) then
     call table_errorappend(crc250,myname)
     call table_errorappend(crc250," Error return from table_maketableFile.")
     call table_errorappendi(crc250,irc)
     call table_errorappend(crc250,"\n")
     return
  end if
  return
end subroutine tab_maketablefile
