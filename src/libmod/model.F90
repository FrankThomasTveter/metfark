 ...
 ! global dimension is used?
  ...
  subroutine model_colocateTable(css,nloc,loc,f,bok,crc250,irc)
    ...
    if (bok) then
       ...
       call model_makeTableFile(css,nloc,loc,f,p,bok,crc250,irc)
       if (irc.ne.0) then
          bok=.false.
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250," Error return from makeTableFile.")
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
  ...
  !###############################################################################
  !
  subroutine model_makeTableFile(css,nloc,lp,f,p,bok,crc250,irc)
    type(mod_session), pointer :: css ! current session
    integer :: nloc                   ! number of locations
    ...
    integer :: irc
    type(mod_batch), pointer :: b
    character*25 :: myname="model_makeTableFile"
    integer :: ii,ll,varid
    ...
    if(mod_bdeb)write(*,*)myname,' Done.'
    return
  end subroutine model_makeTableFile
  !
