...
# include "netcdf.inc"
  ...
  type :: mod_default
     ...
     type(mod_default), pointer :: prev => null()   ! linked list
     type(mod_default), pointer :: next => null()   ! linked list
  end type mod_default
  !
  ! expression rules for model targets and observation target expressions
  !
  type :: mod_expression
     integer :: nTrgExp = 0
     logical, pointer :: vset(:)                         ! is value set?
  ...
  type :: mod_session
     ...
     integer :: oloc = 0                                 ! offset between locid and position in locdata
...
CONTAINS
  ...
  subroutine model_removeSession(css,crc250,irc)
    ...
    type(mod_default), pointer :: cdef, cdefn
    type(mod_expression), pointer :: cexp, cexpn
    integer :: ii
    character*25 :: myname = "model_removeSession"
    ! remove location stack
    if(bdeb)write(*,*)myname,'Entering.',irc
    if(bdeb)write(*,*)myname,'Un-slice.'
    ...
    if (associated(css%firstTrg)) then
       ...
       do while (.not.associated(ctrg,target=css%lastTrg))
          ctrgn => ctrg%next
          call model_deleteTrg(css,ctrg,crc250,irc)
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
    ...
    if (associated(css%firstDef)) then
       ...
       do while (.not.associated(cdef,target=css%lastDef))
          cdefn => cdef%next
          call model_deleteDef(css,cdef,crc250,irc)
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
             call model_errorappend(crc250," Error return from deleteDef.")
             call model_errorappendi(crc250,irc)
             call model_errorappend(crc250,"\n")
             return
          end if
    ...
    !
    ! remove expression stack
    if (associated(css%firstExp)) then
       cexp => css%firstExp%next
       do while (.not.associated(cexp,target=css%lastExp))
          cexpn => cexp%next
          call model_deleteExp(css,cexp,crc250,irc)
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
             call model_errorappend(crc250," Error return from deleteExp.")
             call model_errorappendi(crc250,irc)
             call model_errorappend(crc250,"\n")
             return
          end if
       ...
       deallocate(css%firstExp,css%lastExp)
       nullify(css%currentExp)
    end if
    !
    if(bdeb)write(*,*)myname,'Un-link.'
  ...
  subroutine model_peek(css,maxrep,nrep, rep250, crc250,irc)
    ...
    if (.not.associated(currentFile,target=css%firstFile)) then
       ...
       do ii=1,currentFile%nvar
          ...
          if(bdeb)write(*,*)myname,'VAR80:',var80(1:lenv),lenv
  ...
  ! clear the MODEL POS
  !
  subroutine model_locclear(css,nslice,slice,crc250,irc)
    type(mod_session), pointer :: css !  current session
    integer :: nslice
  ...
  !
  subroutine model_locpush(css,locid,nsliceTarget,sliceTarget,bok,crc250,irc)
    type(mod_session), pointer :: css !  current session
    integer :: locid
    ...
    css%locReady=.false.
    if (css%tloc+css%oloc .ne. locid) then
       irc=346
       call model_errorappend(crc250,myname)
       call model_errorappend(crc250," Non-sequential locid:")
       call model_errorappendi(crc250,locid)
       call model_errorappend(crc250,"<>")
       call model_errorappendi(crc250,css%tloc+css%oloc)
       call model_errorappend(crc250,"\n")
       return
    end if
    if(bdeb)write(*,*)myname,' Done.'
  ...
  subroutine model_locSearchOk(css,locid,bok)
    ...
    if (css%locReady) then
       bok=(css%locData(locid-css%oloc)%ptr%bok .and. css%locData(locid-css%oloc)%ptr%search.eq.0)
    else
       bok=.false.
    end if
    return
  ...
  !
  subroutine model_deleteTrg (css,trg, crc250,irc)
    type(mod_session), pointer :: css !  current session
    type(mod_target), pointer :: trg
    character*250 :: crc250
    integer :: irc  ! error return code (0=ok)
    character*25 :: myname = "model_deleteTrg"
    ...
    css%trg_set=.false.
    return
  end subroutine model_deleteTrg
  !
  ! delete default from stack
  !
  subroutine model_deleteDef (css,def, crc250,irc)
    type(mod_session), pointer :: css !  current session
    type(mod_default), pointer :: def
    character*250 :: crc250
    integer :: irc  ! error return code (0=ok)
    character*25 :: myname = "model_deleteDef"
    ...
    return
  end subroutine model_deleteDef
  !
  ! delete expression rule from stack
  !
  subroutine model_deleteExp (css,exp, crc250,irc)
    type(mod_session), pointer :: css !  current session
    type(mod_expression), pointer :: exp
    character*250 :: crc250
    integer :: irc  ! error return code (0=ok)
    character*25 :: myname = "model_deleteExp"
    integer :: ii
    if (associated(exp)) then
       css%nexp = css%nexp - 1
       exp%next%prev => exp%prev
       exp%prev%next => exp%next
       if (associated(exp%vset)) deallocate(exp%vset)
       if (associated(exp%n80)) deallocate(exp%n80)
       if (associated(exp%lenn)) deallocate(exp%lenn)
       if (associated(exp%e250)) deallocate(exp%e250)
       if (associated(exp%lene)) deallocate(exp%lene)
       if (associated(exp%l80)) deallocate(exp%l80)
       if (associated(exp%u80)) deallocate(exp%u80)
       if (associated(exp%val)) deallocate(exp%val)
       if (associated(exp%psp)) then
          do ii=1,exp%nTrgExp
             call parse_close(exp%psp(ii)%ptr,crc250,irc)
          end do
          deallocate(exp%psp)
       end if
       exp%nTrgExp=0
    ...
    return
  end subroutine model_deleteExp
  !
  !###############################################################################
  ...
  ! Used in this way
  ! 1)  model_orderstack: make index of all analysis in stack
  ...
  subroutine model_orderstack(css,crc250,irc)
    ...
    return
  end subroutine model_orderstack
  ...
  subroutine model_getnextfile(css,ind_lim,ind_start,ind_stop,bok,crc250,irc)
    ...
    SEARCH : do while (.not.bdone)
       if (css%currentFile%ind_lim .and. css%ind_lim) then
          if ((css%currentFile%ind_start.le.css%ind_stop .and.css%currentFile%ind_start.ge.css%ind_start) .or.  &
             ...
             if(bdeb)write(*,*)myname,'Found file with valid range.'
    ...
    if (allocated(sort)) deallocate(sort)
    if(bdeb)write(*,*)myname,'Done.',irc,bok
    return
  ...
  character*250 function model_getGrid250(newFile,v,loc,val,wgt,crc250,irc)
    ...
    character*250 :: crc250
    integer :: irc
    character*25 :: myname = "model_getXML250"
    ...
    if (val.eq.nf_fill_double) then
       if(bdeb)write(*,*)myname,'Match:',val,nf_fill_double,val.eq.nf_fill_double,val-nf_fill_double
       val50=""
       lenv=0
    else
       if(bdeb)write(*,*)myname,'No match:',val,nf_fill_double,val.eq.nf_fill_double,val-nf_fill_double
  ...
  character*250 function model_getInt250(newFile,v,loc,val,crc250,irc)
    ...
    character*250 :: crc250
    integer :: irc
    character*25 :: myname = "model_getXMLWgt250"
    integer ::lenv,ii
    character*50 :: val50
    character*10 :: pos10
    integer, external :: length
...
!!!!! write(*,*) myname,j2000,model_gettime
  ...
  character*250 function model_pretty(var80,ndims,dim80,istart,istop)
    ...
    do ii=1,ndims
       ...
       if(bdeb)write(*,*) "model_pretty  dim80:",dim80(ii)(1:lend),istart(ii),istop(ii)
       if (istop(ii).gt.1) then
          write(yuff250,*)istop(ii);call chop0(yuff250,250);lenx=length(yuff250,250,2)
          write(xuff250,'(I8,"+",A)')istart(ii),yuff250(1:lenx);call chop0(xuff250,250);lenx=length(xuff250,250,2)
       else
          write(xuff250,'(I8)')istart(ii);call chop0(xuff250,250);lenx=length(xuff250,250,2)
       end if
       xuff250=dim80(ii)(1:lend)//"["//xuff250(1:lenx)//"]";call chop0(xuff250,250);lenx=length(xuff250,250,2)
  ...
  subroutine model_readInventory(css,newFile,crc250,irc)
    ...
    !
    if(bdeb)write(*,*)myname,' Allocate file.'
    ...
    !     
    if(bdeb)write(*,*)myname,' Store dim.'
    ...
    !
    if(bdeb)write(*,*)myname,' Allocate var.',newFile%nvar
    !     allocate variables in file
    allocate(newFile%var(newFile%nvar),stat=irc)
    if (irc.ne.0) then
       call model_errorappend(crc250,myname)
    ...
    if(bdeb)write(*,*)myname,' Store var.'
    ...
    do varid=1,newFile%nvar
       ...
       if (ret .ne. NF_NOERR) then
          irc=802
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Error return from NF_INQ_VARNDIMS."//nf_strerror(ret))
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
       ...
       !
       if(bdeb)write(*,*)myname,' Store attr.'
       ...
       do ii=1,newFile%var(varid)%ptr%natt
          ...
          if(bdeb)write(*,*)myname,' get attr.',ii,att%att80(1:att%lena)
  ...
  subroutine model_readVariable(css,newFile,varid,crc250,irc)
    ...
    integer :: ii,ret
    !
    if(bdeb)write(*,*)myname,'Entering. ',newFile%var(varid)%ptr%var80(1:newFile%var(varid)%ptr%lenv)
    ! find length of grid
    newFile%var(varid)%ptr%len=1
    do ii=1,newFile%var(varid)%ptr%ndim
       newFile%var(varid)%ptr%len=newFile%var(varid)%ptr%len*(newFile%istop(newFile%var(varid)%ptr%ind(ii)) &
    ...
    if (newFile%var(varid)%ptr%type.eq.nf_int2) then
       ...
       if (ret .ne. NF_NOERR) then
          irc=812
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Error return from NF_GET_VARA_INT2."//nf_strerror(ret))
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
       ...
       if (ret .ne. NF_NOERR) then
          irc=812
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Error return from NF_GET_VARA_INT."//nf_strerror(ret))
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
       ...
       if (ret .ne. NF_NOERR) then
          irc=812
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Error return from NF_GET_VARA_REAL."//nf_strerror(ret))
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
       ...
       do ii=1,newFile%var(varid)%ptr%len
          if (newFile%var(varid)%ptr%fr(ii).eq.fillr) then
             ...
             newFile%var(varid)%ptr%fd(ii)=newFile%var(varid)%ptr%scale*newFile%var(varid)%ptr%fr(ii)
          end if
       end do
       newFile%var(varid)%ptr%scale=1.0D0
       ...
       if(bdeb)write(*,*)myname,'Undefined.'
    ...
    if(bdeb)write(*,*)myname,'Done. ',newFile%var(varid)%ptr%type,&
  ...
  subroutine model_clearVariable(v,crc250,irc)
    ...
    if(bdeb)write(*,*)myname,'Entering.',associated(v)
  ...
  subroutine model_deleteAttributes(v,crc250,irc)
    ...
    integer :: attid
    if(bdeb)write(*,*)myname,'Entering.',associated(v)
    if(bdeb)write(*,*)myname,'Have.',v%natt,associated(v%att)
    do attid=1,v%natt
       if(bdeb)write(*,*)myname,'Loop.',attid,v%natt,associated(v%att)
  ...
  subroutine model_clearAttribute(att,crc250,irc)
    ...
    if(bdeb)write(*,*)myname,'Entering.',associated(att)
    if (allocated(att%ac)) deallocate(att%ac)
    if (allocated(att%a1)) deallocate(att%a1)
    if (allocated(att%a2)) deallocate(att%a2)
    if (allocated(att%a4)) deallocate(att%a4)
...
!          write(*,*)myname,'Checking: ',ii,' "',newFile%var80(ii)(1:newFile%lenv(ii)),'"  "',css%ind_var80(1:css%ind_lenv),'"'
       ...
       if (iisort .ne. 0) then
          ...
          if (newFile%nsort .lt.1000) then
             ...
             irc=453
             call model_errorappend(crc250,myname)
             call model_errorappend(crc250," Invalid "//trim(newFile%var(iisort)%ptr%var80)//" dimensions.")
          ...
          if(bdeb)write(*,*)myname,'Unable to find sort variable: "',css%ind_var80(1:css%ind_lenv),'"'
       end if
    end if
  ...
  subroutine model_closeFile(css,newFile,crc250,irc)
    ...
    if (ret .ne. NF_NOERR) then
       irc=170
       call model_errorappend(crc250,myname)
       call model_errorappend(crc250," Unable to close: "//newFile%fn250(1:newFile%lenf)//" "//nf_strerror(ret))
       call model_errorappendi(crc250,irc)
       call model_errorappend(crc250,"\n")
       return
    end if
  ...
  subroutine model_slicecurrentfile(css,bok,crc250,irc)
    ...
    ! make location array
    call model_locget(css,css%locData,crc250,irc)
    if (irc.ne.0) then
       call model_errorappend(crc250,myname)
    ...
    call model_slicefile(css,css%tloc,css%locData,css%currentFile,bok,crc250,irc)
    if (irc.ne.0) then
       call model_errorappend(crc250,myname)
  ...
  !
   subroutine model_locget(css,loc,crc250,irc)
    type(mod_session), pointer :: css !  current session
    type(mod_locPointer), allocatable :: loc(:)
    character*250 :: crc250
    integer :: irc
    type(mod_location), pointer :: currentLoc => null()
    character*25 :: myname = "model_locget"
    integer :: ii
    if (associated(css%firstLoc).and..not.css%locready.and.css%tloc.gt.0) then
       if (allocated(loc)) deallocate(loc)
       allocate(loc(css%tloc),stat=irc)
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
  ...
  subroutine model_slicefile(css,nloc,loc,f,bok,crc250,irc)
    ...
    if (bok) then
       ...
       call model_makeXML(css,nloc,loc,f,p,bok,crc250,irc)
       if (irc.ne.0) then
          bok=.false.
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250," Error return from makeXML.")
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
  ...
  !
  ! make plan for target selection
  !
  subroutine model_planBatch(css,newFile,p,crc250,irc)
    ...
    !
    if(bdeb)write(*,*)myname,'Entering.',css%ngslice,newFile%nvar
    ...
    if(bdeb)write(*,*)myname,'Find batch jobs.'
    !
    ...
    do ii=1,css%ngslice
       if(bdeb)write(*,*)myname,'Batch slice loop.',ii,css%ngslice
       if (.not.sliceProcessed(ii)) then ! slice not searched yet
          ...
          if (p%indvar(ii).ne.0) then ! this is a variable
             ...
             if(bdeb)write(*,*)myname,'Found Var:',ii,newFile%var80(p%indvar(ii))(1:10)
             ...
             do kk=1,var%ndim
                if (.not. markDim(var%ind(kk))) then
                   ...
                   if(bdeb)write(*,*)myname,'Mark Dim:',ii,newFile%dim80(var%ind(kk))(1:10)
                   b%ndim=b%ndim+1
             ...
             if(bdeb)write(*,*)myname,'Found Dim:',ii,newFile%dim80(p%inddim(ii))(1:10)
          ...
          if(bdeb)write(*,*)myname,'Cross-checking.',changed,b%ndim,b%nvar
          LOOP: do while (changed)
             ...
             do jj=ii+1,css%ngslice
                if (.not.sliceProcessed(jj)) then ! not processed yet
                   if (p%indvar(jj).ne.0) then ! is a variable
                      ...
                      if(bdeb)write(*,*)myname,'>>Found Var:',ii,newFile%var80(p%indvar(jj))(1:10)
             ...
             if(bdeb)write(*,*)myname,'Found Dim:',ii,newFile%dim80(p%inddim(jj))(1:10)
          ...
          if(bdeb)write(*,*)myname,'Store batch.',ii,css%ngslice
          ! add batch jobb...
          if(bdeb)write(*,*)myname,'Allocating.',b%ndim,b%nvar
          allocate(b%ind(b%ndim),b%inc(b%ndim),b%slidim(b%ndim),b%trgdim(b%ndim),b%dim80(b%ndim),&
               & b%var(b%nvar),b%val(b%nvar),b%proc(b%nvar),b%slivar(b%nvar),b%trgvar(b%nvar),b%var80(b%nvar),&
               & stat=irc)
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
          ...
          if(bdeb)write(*,*)myname,'Store variables.',b%ndim,b%nvar,allocated(newFile%var80)
          do jj=1,b%nvar
          ...
          if(bdeb)write(*,*)myname,'Store dimensions.',b%ndim,b%nvar,allocated(newFile%dim80)
          do jj=1,b%ndim
             b%ind(jj)=inddim(jj)
             b%inc(jj)=1
             b%slidim(jj)=slidim(jj) ! position in slice target array, 0 if none
             b%trgdim(jj)=0.0D0
             if(bdeb)write(*,*)myname,'Store dimension loop.',jj,inddim(jj)

             if(bdeb)write(*,*)myname,'Dim80:',b%ind(jj),newFile%dim80(b%ind(jj))(1:10)

             b%dim80(jj)=newFile%dim80(b%ind(jj))
          end do
          if(bdeb)write(*,*)myname,'Store in plan chain.'
          ! store in plan....
          b%prev => p%last%prev
          p%last%prev%next => b
          b%next => p%last
  ...
  subroutine model_clearPlan(p,crc250,irc)
    ...
    if(bdeb)write(*,*)myname,'Entering.',associated(p)
    if (associated(p)) then
       ...
       !
       if(bdeb)write(*,*)myname,'Deleting batch jobs.',associated(p%first)
       b => p%first%next
  ...
  subroutine model_printBatch(b,crc250,irc)
    ...
    integer, external :: length
    !
    if(bdeb)write(*,*)myname,'Entering.',irc,b%ndim,b%nvar
    buff250="Dims=("
    call chop0(buff250,250)
  ...
  subroutine model_initPlan(p,css,newFile,crc250,irc)
    ...
    !
    if(bdeb)write(*,*)myname,'Entering.',associated(p),allocated(p%inddim),associated(p%last),irc
    p%nvar=newFile%nvar
    p%ndim=newFile%ndim
    allocate(p%markDim(p%ndim),p%trgvar(newFile%nvar),p%hastrg(newFile%nvar),p%indvar(css%ngslice),&
    ...
    !
    if(bdeb)write(*,*)myname,'Analysing slice variables.'
    do ii=1,css%ngslice
          ...
          if (css%gslice80(ii)(1:1).eq."(".and.css%gslice80(ii)(leng:leng).eq.")") then
             do jj=1,newFile%ndim
                ...
                if (css%gslice80(ii)(2:leng-1).eq.newFile%dim80(jj)(1:lend)) then
                   ...
                   if(bdeb)write(*,*) myname,'Dim: "'//css%gslice80(ii)(2:leng-1)//'"  "'//newFile%dim80(jj)(1:lend)//'"',ii,jj
       ...
       if (p%inddim(ii).eq.0) then ! not a dimension, must be a variable
          do jj=1,newFile%nvar ! global variable index
             ...
             if (css%gslice80(ii)(1:leng).eq.newFile%var80(jj)(1:lenv)) then
                p%indvar(ii)=jj
                if(bdeb)write(*,*) myname,'Var: "'//css%gslice80(ii)(1:leng)//'"  "'//newFile%var80(jj)(1:lenv)//'"',ii,jj
             end if
          end do
       end if
  ...
  !
 subroutine model_makeXML(css,nloc,loc,newFile,p,bok,crc250,irc)
    type(mod_session), pointer :: css !  current session
    integer :: nloc                 ! number of locations
    type(mod_locPointer) :: loc(nloc)   !  location pointer
    type(mod_file),pointer :: newFile
    ...
    !
    if(bdeb)write(*,*)myname,'Batch job loop.'
    b=>p%first%next
    do while ( .not.associated(b,target=p%last))
       !
       ...
       !
       if(bdeb)write(*,*)myname,'Location loop.',nloc,b%nvar
       do ll=1,nloc
          if (loc(ll)%ptr%bok) then
             if (loc(ll)%ptr%search .eq. 0) then
                ...
                if (bdeb) then
                   call model_writeXMLSummary(css,newFile,loc(ll)%ptr,p,crc250,irc)
                   if (irc.ne.0) then
                      call model_errorappend(crc250,myname)
                      call model_errorappend(crc250," Error return from writeXMLSummary.")
                      call model_errorappendi(crc250,irc)
                      call model_errorappend(crc250,"\n")
                      return
                   end if
    ...
    do while ( .not.associated(b,target=p%last))
       ...
       !
       if(bdeb)write(*,*)myname,'Variable loop.',b%nvar
       do ii = 1, b%nvar 
          varid=b%var(ii)
          p%proc(varid)=.true.
          if(bdeb)write(*,*)myname,'Processed:',varid,p%proc(varid)
          dim50=model_getDim(newFile,newFile%var(varid)%ptr)
          call chop0(dim50,50)
          lend=length(dim50,50,10)
          if (lend.eq.0) then
             var250="variable='"//newFile%var(varid)%ptr%var80(1:newFile%var(varid)%ptr%lenv)//"'"
          else
             var250="variable='"//newFile%var(varid)%ptr%var80(1:newFile%var(varid)%ptr%lenv)&
                  & //"' dim='"//dim50(1:lend)//"'"
          end if
          ...
          if(bdeb)write(*,*)myname,'Variable:'//var250(1:lenv)
          do ll=1,nloc
             if (loc(ll)%ptr%bok) then
                call model_getTarget50(loc(ll)%ptr%sliceTarget(b%slivar(ii)),lent,trg50)
                !
                call model_writeXML(css,newFile,varid,lenv,var250,lent,trg50,loc(ll)%ptr,p,bok,crc250,irc)
                if (irc.ne.0) then
                   call model_errorappend(crc250,myname)
                   call model_errorappend(crc250," Error return from writeXML.")
                   call model_errorappendi(crc250,irc)
                   call model_errorappend(crc250,"\n")
                   return
                end if
          ...
          if(bdeb)write(*,*)myname,'Processed done:',p%proc(1)
       end do
       !
    ...
    !
    if(bdeb)write(*,*)myname,'Looping over remaining variables.',newFile%nvar, p%nvar
    !
  ...
  !
  subroutine model_writeXMLSummary(css,newFile,loc,p,crc250,irc)
    type(mod_session), pointer :: css   ! current session
    type(mod_file),pointer :: newFile   ! current file
    type(mod_location),pointer :: loc   ! location
    ...
    character*250 :: crc250
    integer :: irc
    character*25 :: myname = "model_writeXMLSummary"
    character*250 :: buff250
    character*50 :: s1
    integer :: lenb,len1
    integer, external :: length
    ...
    return
  end subroutine model_writeXMLSummary
  !
  ! make XML for variables at location
  !
  subroutine model_writeXML(css,newFile,varid,lenv,var250,lent,trg50,loc,p,bok,crc250,irc)
    type(mod_session), pointer :: css   ! current session
    type(mod_file),pointer :: newFile   ! current file
    ...
    if(bdeb)write(*,*)myname,"Undef:",nf_fill_double
    !
    ...
    do while (.not. bout)
       ...
       binn=.false.
       do while (.not. binn)
          if(bdeb)write(*,*)myname,"Calling getValue."
          ...
          if(bdeb)write(*,*)myname,"Calling getXML."
          cnt=cnt+1
          ctot=ctot+1
          if(bdeb)write(*,*)myname,"Calling Incrementing position."
          if (val.ne.nf_fill_double) then
             if (first) then
                write(*,'(3X,A)') "<loc "//loc250(1:lenl)//" "//var250(1:lenv)//pos250(1:lenp)//">"
                first=.false.
             end if
             wgt=model_getWeight(ninn,inn,loc%ndim,loc%pos,loc%istart,loc%istop,loc%ff) ! current weight
             buff250=model_getGrid250(newFile,v,loc,val,wgt,crc250,irc)
             if (irc.ne.0) then
                call model_errorappend(crc250,myname)
                call model_errorappend(crc250," Error return from getGridXml.")
                call model_errorappendi(crc250,irc)
                call model_errorappend(crc250,"\n")
                return
             end if
       ...
       if (twgt.gt.1.0D-10.and.ctot.eq.cval.and.ctot.gt.1) then
          ...
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
             call model_errorappend(crc250," Error return from getIntXml.")
             call model_errorappendi(crc250,irc)
             call model_errorappend(crc250,"\n")
             return
          end if
       ...
       bout=(cnt.gt.32.or.(model_incrementPos(nout,out,loc%ndim,loc%pos,loc%istart,loc%istop).eq.0))
       if(bdeb)write(*,*)'Count:',cnt
    end do
    if (.not.first) then
       write(*,'(3X,A)') "</loc>"
  ...
  subroutine model_initLocPos(css,newFile,p,b,loc,crc250,irc)
    ...
    ! loc
    !
    if(bdeb)write(*,*)myname,'Entering.',newFile%ndim,allocated(newFile%istop)
    if(bdeb)write(*,*)myname,'Is loc associated:',associated(loc)
    if (allocated(loc%pos)) deallocate(loc%pos)
    if (allocated(loc%istart)) deallocate(loc%istart)
    if (allocated(loc%istop)) deallocate(loc%istop)
    if (allocated(loc%ff)) deallocate(loc%ff)
    loc%ndim=newFile%ndim
    allocate(loc%pos(loc%ndim),loc%istart(loc%ndim),loc%istop(loc%ndim),loc%ff(loc%ndim),stat=irc)
    if (irc.ne.0) then
       call model_errorappend(crc250,myname)
  ...
  subroutine model_search(css,newFile,p,b,loc,crc250,irc)
    ...
    !
    if(bdeb)write(*,*) myname,"Vars:",(" "//newFile%var80(b%var(ii))(1:newFile%lenv(b%var(ii))),ii=1,b%nvar)
    if(bdeb)write(*,*) myname,"Dims:",(" "//newFile%dim80(b%ind(ii))(1:newFile%lend(b%ind(ii))),ii=1,b%ndim)
    !
  ...
  subroutine model_getTarget(newFile,p,b,loc,crc250,irc)
    ...
    do ii=1,b%nvar
       if (b%slivar(ii).ne.0) then
          ...
          irc=347
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250," Missing target variable:"//newFile%var80(b%var(ii)) )
          call model_errorappendi(crc250,irc)
          call model_errorappend(crc250,"\n")
          return
       end if
    ...
    do ii=1,b%ndim
       if (b%slidim(ii).ne.0) then
          ...
          if(bdeb)write(*,*) myname,">>>>>>>Loc:",ii,loc%pos(b%ind(ii)),loc%istart(b%ind(ii)),loc%istop(b%ind(ii))
  ...
  subroutine model_getIncrement(newFile,b,loc,nv,crc250,irc)
    ...
    do jj=1,b%ndim
       if (b%slidim(jj).eq.0) then 
          ...
          if(bdeb)write(*,*)myname,'D(var)=',(v(ii,jj),ii=1,b%nvar)
    ...
    do jj=1,b%ndim ! desired dimension
       if (b%slidim(jj).eq.0) then 
          ...
          if(bdeb)write(*,*)myname,'N(dim)=',jj,dot_nv,dot_vv,dot_nv/dot_vv
  ...
  subroutine model_useIncrements(newFile,b,loc,nv,changed,crc250,irc)
    ...
    changed=.false.
    do jj=1,b%ndim
       if(bdeb)write(*,*)myname,'Flags:',jj,(b%slidim(jj).ne.0),nv(jj,0),nv(jj,jj)
       if (b%slidim(jj).ne.0) cycle ! target is already set...
       if (nv(jj,0) .lt. 0) then ! decrement
          if (loc%pos(b%ind(jj)).gt.1) then
             loc%pos(b%ind(jj))=max(1,loc%pos(b%ind(jj))-max(1,nint(-nv(jj,0)*0.61803399D0+0.5D0)))! -1
             changed=.true.
          end if
       else if (nv(jj,jj) .lt. 0) then ! increment
          if (loc%pos(b%ind(jj)).lt.newFile%istop(b%ind(jj))-1) then
             loc%pos(b%ind(jj))=min(newFile%istop(b%ind(jj))-1,loc%pos(b%ind(jj))+max(1,-nint(nv(jj,jj)*0.61803399D0+0.5D0))) ! +1
             changed=.true.
          end if
       else
  ...
  real function model_getValue(newFile,v,loc,crc250,irc)
    ...
    if (v%ndim.gt.0) then
       ...
       if(bdeb)write(*,*)myname,"Pos="//pos50(1:lenp)//" ll="//loc50(1:lenl)//" val="//val50(1:lenv)
    else
  ...
  character*50 function model_getDim(file,var)
    ...
    do ii=1,var%ndim
       if (leng.eq.0)then
          ...
          model_getDim=model_getDim(1:leng)//","//file%dim80(var%ind(ii))(1:file%lend(var%ind(ii)))
  ...
  real function model_getWeight(n,ind,m,ipos,istart,istop,wgt) 
    ...
    do ii=1,n
       ...
       if (di.gt.0) then
          ...
          if(bdeb)write(*,*)myname,'Weight:',ii,ww,wgt(ind(ii)),ipos(ind(ii)),istart(ind(ii))
  ...
  integer function model_targetCount(css,crc250,irc)
    ...
    model_targetCount=css%ntarget
    return
  end function model_targetCount
  !
  ! get number of expressions
  !
  integer function model_expressionCount(css,crc250,irc)
    type(mod_session), pointer :: css   ! session structure
    character*250 :: crc250
    integer :: irc
    character*22 :: myname = "model_expressionCount "
    if (associated(css%currentExp)) then
       model_expressionCount=css%currentExp%nTrgExp
    else
       model_expressionCount=0
    end if
    return
  end function model_expressionCount
  !
  ! default locations
  !
  integer function model_defaultCount(css,crc250,irc)
    type(mod_session), pointer :: css   ! session structure
    character*250 :: crc250
    integer :: irc
    character*22 :: myname = "model_defaultCount "
    model_defaultCount=css%ndef
    return
  end function model_defaultCount
  !
  ...
  end subroutine model_cleardefault
  !
...
!    if (css%ctarget.eq.0) then
       css%ctarget=css%ntarget
       if(associated(css%trg80)) deallocate(css%trg80)
       if(associated(css%trg_lent)) deallocate(css%trg_lent)
       if(associated(css%trg_v80)) deallocate(css%trg_v80)
       if(associated(css%trg_l80)) deallocate(css%trg_l80)
       ...
       if(associated(css%trg_ook)) deallocate(css%trg_ook)
       if(associated(css%trg_orm)) deallocate(css%trg_orm)
       if (css%ctarget.ne.0) then
          allocate(css%trg80(css%ctarget), css%trg_lent(css%ctarget), css%trg_v80(css%ctarget),  &
               & css%trg_l80(css%ctarget), css%trg_u80(css%ctarget), &
               & css%trg_minval(css%ctarget), css%trg_maxval(css%ctarget), &
               & css%trg_minset(css%ctarget), css%trg_maxset(css%ctarget), &
               & css%trg_val(css%ctarget), &
               & css%trg_ook(0:css%ctarget), css%trg_orm(0:css%ctarget), stat=irc)
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
 ...
 !
  ! set target values given expression values
  !
  subroutine model_setTargetVal(css,crc250,irc)
    type(mod_session), pointer :: css !  current session
    character*250 :: crc250
    integer :: irc
    ...
    if (associated(css%currentDef)) then
       do ii=1,css%ctarget
          css%trg_val(ii)=css%currentDef%val(ii)
       end do
    end if
    if (associated(css%currentExp)) then
       do ii=1,css%ctarget
          css%trg_val(ii)=css%currentExp%val(ii)
       end do
    end if
    return
  ...
  subroutine model_writeSummary(css,crc250,irc)
    ...
    if (css%trg_orm(0).ne.0) then
       write(*,'(4X,A,I0,A)')"<check removed='",css%trg_orm(0),"' reason='outside target limits.'>"
    end if
    do ii=1,css%ctarget
  ...
  ! compile expression
  !
  subroutine model_compileExpression(css,ii,var80,crc250,irc)
    type(mod_session), pointer :: css !  current session
    integer :: ii ! expression number
    character*80, allocatable :: var80(:) ! variables
    character*250 :: crc250
    integer :: irc
    character*25 :: myname = "model_compileExp"
    type(mod_expression), pointer :: exp
    integer :: lene
    if(bdeb)write(*,*)myname,'Entering.',ii,var80
    exp => css%currentExp
    if (associated(exp)) then
       lene=exp%lene(ii)
       if(bdeb)write(*,*)myname,'Calling parsef.'
       call parse_parsef(exp%psp(ii)%ptr,exp%e250(ii)(1:lene),var80,crc250,irc)
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
       ...
       irc=342
       call model_errorappend(crc250,myname)
       call model_errorappend(crc250," No expressions specified.")
       call model_errorappendi(crc250,irc)
       call model_errorappend(crc250,"\n")
       return
    end if
    return
  end subroutine model_compileExpression
  !
  ! evaluate expression
  !
  subroutine model_evalExpression(css,ii,val,crc250,irc)
    type(mod_session), pointer :: css !  current session
    integer :: ii                     ! expression number
    real, allocatable :: val(:)    ! values
    character*250 :: crc250
    integer :: irc
    character*25 :: myname = "model_evalExp"
    type(mod_expression), pointer :: exp
    exp => css%currentExp
    exp%val(ii)=parse_evalf(exp%psp(ii)%ptr,val)
    return
  end subroutine model_evalExpression
  !
  ! add default element
  !
  subroutine model_addDefault(css,n80,v80,crc250,irc)
    ...
    if(bdeb)write(*,*)myname,'Here.',associated(css%currentDef)
    if (.not.associated(css%currentDef)) then
       allocate(css%currentDef, stat=irc)
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Unable to allocate 'firstItm/lastItm'.")
          call model_errorappend(crc250,"\n")
          return
       end if
       css%currentDef%nTrgDef=css%ctarget
       allocate(css%currentDef%vset(css%currentDef%nTrgDef), css%currentDef%v80(css%currentDef%nTrgDef), &
            & css%currentDef%vlen(css%currentDef%nTrgDef),  css%currentDef%val(css%currentDef%nTrgDef), stat=irc)
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Unable to allocate 'session: current Default'.")
          call model_errorappend(crc250,"\n")
          return
       end if
    end if
    ...
    if(bdeb)write(*,*)myname,'Looking for target:',n80(1:lenn)
    ii=1
    SEEK:do while (ii.le.css%ctarget)
       if (css%trg80(ii)(1:css%trg_lent(ii)).eq.n80(1:lenn)) exit SEEK
       ii=ii+1
    end do SEEK
    if (ii.le.css%ctarget) then
       ...
       irc=220
       ! write(*,*)myname,'Targets:',css%ctarget,css%ntarget
       ! do ii=1,css%ctarget
       !    write(*,*)myname,'Target:',ii,css%trg80(ii)(1:css%trg_lent(ii))
       ! end do
       call model_errorappend(crc250,myname)
       call model_errorappend(crc250,"Target not found:"//n80(1:lenn))
       return
    end if
    if(bdeb)write(*,*)myname,'Done.',irc
    return
  end subroutine model_addDefault
  !
  ...
  subroutine model_pushDefault(css,crc250,irc)
    ...
    if (.not.associated(css%firstDef)) then
       ...
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Unable to allocate 'firstDef/lastDef'.")
          call model_errorappend(crc250,"\n")
          return
       end if
       css%firstDef%next => css%lastDef
  ...
  subroutine model_clearexpressionstack(css,crc250,irc)
    ...
    do while (.not.associated(currentExpression,target=css%lastExp))
       ...
       currentExpression%prev%next =>  currentExpression%next
       currentExpression%next%prev =>  currentExpression%prev
       deallocate(currentExpression,stat=irc)
       css%nexp=css%nexp-1
  ...
  subroutine model_addexpression(css,n80,e250,l80,u80,crc250,irc)
    ...
    if (.not.associated(css%currentExp)) then
       allocate(css%currentExp, stat=irc)
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Unable to allocate 'firstItm/lastItm'.")
          call model_errorappend(crc250,"\n")
       ...
       css%currentExp%nTrgExp=css%ctarget
       allocate(css%currentExp%vset(css%currentExp%nTrgExp), &
            & css%currentExp%n80(css%currentExp%nTrgExp), &
            & css%currentExp%lenn(css%currentExp%nTrgExp), &
            & css%currentExp%e250(css%currentExp%nTrgExp), &
            & css%currentExp%lene(css%currentExp%nTrgExp), &
            & css%currentExp%l80(css%currentExp%nTrgExp),  &
            & css%currentExp%u80(css%currentExp%nTrgExp), &
            & css%currentExp%val(css%currentExp%nTrgExp), &
            & css%currentExp%psp(css%currentExp%nTrgExp), stat=irc)
       if (irc.ne.0) then
          call model_errorappend(crc250,myname)
          call model_errorappend(crc250,"Unable to allocate 'session: current Expression'.")
          call model_errorappend(crc250,"\n")
          return
       end if
       exp => css%currentExp
       do ii=1,exp%nTrgExp
          exp%lene(ii)=0
          exp%val(ii)=0.0D0
          call parse_open(exp%psp(ii)%ptr,crc250,irc)
          if (irc.ne.0) then
             call model_errorappend(crc250,myname)
    ...
    if (ii.le.css%ctarget) then
       css%currentExp%vset(ii)=.true.
       css%currentExp%n80(ii)=n80
       css%currentExp%lenn(ii)=lenn
       css%currentExp%e250(ii)=e250
       call chop0(css%currentExp%e250(ii),250)
       css%currentExp%lene(ii)=length(css%currentExp%e250(ii),250,10)
       if(bdeb)write(*,*)myname,'Expression.',css%currentExp%e250(ii)(1:css%currentExp%lene(ii))
       css%currentExp%l80(ii)=l80
       css%currentExp%u80(ii)=u80
       css%currentExp%val(ii)=0.0D0
    else
       irc=221
       write(*,*)myname,'Targets:',css%ctarget,css%ntarget
       do ii=1,css%ctarget
  ...
  subroutine model_pushExpression(css,crc250,irc)
    ...
    if (associated(css%currentExp)) then
       ...
       css%nexp=css%nexp+1
       newExpression%prev => css%lastExp%prev
       newExpression%next => css%lastExp
       newExpression%prev%next => newExpression
       newExpression%next%prev => newExpression
       nullify(css%currentExp)
    end if
    if(bdeb)write(*,*) myname,'Done.'
    return
  end subroutine model_pushExpression
  !
  ...
  subroutine model_getvariables(css,nvar,var80,crc250,irc)
    ...
    if (irc.ne.0) then
       ...
       call model_errorappendi(crc250,css%ntarget)
       return
    end if
    nvar=0
    currentTarget => css%firstTrg%next
    do while (.not.associated(currentTarget,target=css%lastTrg))
       nvar=min(css%ntarget,nvar+1)
  ...
  subroutine model_getdefault(css,nvar,var,crc250,irc)
    ...
    return
  end subroutine model_getdefault
  !
  subroutine model_filestartxml(css,crc250,irc)
    ...
    if (associated(css%currentFile)) then
       write(*,'(1X,A)')"<modelFile file='"//css%currentFile%fn250(1:css%currentFile%lenf)//"'>"
    else
