subroutine col_setfilter(sid,filter250, crc250, irc)
  use colocation
  implicit none
  ...
  call colocation_setfilter(css,filter250,crc250,irc)
  if (irc.ne.0) then
     call colocation_errorappend(crc250,"|")
     call colocation_errorappend(crc250,trim(myname))
     call colocation_errorappend(crc250," Error return from col_setobscache.")
     call colocation_errorappendi(crc250,irc)
     return
  end if
  !write(*,*) myname,' Done.'
  return
end subroutine col_setfilter
