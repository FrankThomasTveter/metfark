...
#
my %dirs = ( obs => {"/elysium/metfark/obs/"      => "rw" },
...
my $autoDir="$pub/auto/"; check_path($autoDir);
...
sub saveModel {
	...
	if ($priv eq "rw" || $priv eq "ro") {
	    ...
	    &term("Permission denied: '".$dir."'");
	}
    } else {
	&term("Invalid directory: '".$adir."'");
    }
}
sub saveObs {
    my ($param) = @_;
...
sub savePlot {
	...
	if ($priv eq "rw" || $priv eq "ro") {
	    ...
	    $node->setAttribute("password",        $password);
	    $node->setAttribute("file",            $base);
	    $node->setAttribute("output",          $output);
...
sub saveAuto {
    ...
    my $base=basename($param->{root}[0]);
    my $idir=dirname($param->{root}[0]);
    my $cls = "auto";
    my $adir=getAdir($cls,$idir);
    if (defined $adir && -d $adir) {
	my $dir = abs_path( $adir ) . "/" ;
	...
	if ($priv eq "rw" || $priv eq "ro") {
		...
		foreach my $line (@lines) {
			...
			if ($len == 2) {
			    ...
			    $parent->setAttribute("lastAuto",$items[1]);
